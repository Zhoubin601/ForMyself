...
test('关系更新同时整理三类记忆、未完话题和女朋友状态', () => {
  ...
  assert.equal(parsed.memoryUpserts.length, 3)
  assert.deepEqual(parsed.memoryUpserts.map(item => item.scope).sort(), ['companion', 'relationship', 'user'])
