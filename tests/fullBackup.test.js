...
test('完整备份包含四类数据和设置，但不包含主密码或生物识别凭据', () => {
  ...
  assert.deepEqual(getFullBackupCounts(snapshot), { savings: 1, weight: 1, mood: 1, passwords: 1 })
...
test('完整恢复在写入前拒绝错误类型、版本、日期和缺失数据集', () => {
  ...
  assert.throws(() => normalizeFullBackupSnapshot({ ...snapshot, version: 2 }), /UNSUPPORTED_FULL_BACKUP_VERSION/)
