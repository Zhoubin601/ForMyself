...
const normalizeCategory = value => String(value || '').trim().slice(0, 20) || DEFAULT_VAULT_CATEGORY
...
export function getVaultCategories(records = []) {
  const custom = records
    .map(record => normalizeCategory(record.category))
    .filter(category => !BUILT_IN_VAULT_CATEGORIES.includes(category))
  return [...BUILT_IN_VAULT_CATEGORIES, ...new Set(custom)]
}
